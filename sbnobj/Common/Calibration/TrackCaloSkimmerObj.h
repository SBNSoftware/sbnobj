...
namespace sbn {
  ...
  struct TrackInfo {
    ...
    double t0[3]; ///!< T0s associated with this track. Index 0 is Pandora, 1 is CRTTrack (SBND like) 2 is CRTHit (ICARUS like)
    ...
    TrackInfo():
      t0{std::numeric_limits<float>::signaling_NaN(),std::numeric_limits<float>::signaling_NaN(),std::numeric_limits<float>::signaling_NaN()},
      id(-1),
      cryostat(-1),
