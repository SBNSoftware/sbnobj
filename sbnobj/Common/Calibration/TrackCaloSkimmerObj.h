...
namespace sbn {
  ...
  struct HitInfo {
    float integral = float(-1.f);    //!< Integral of gaussian fit to ADC values in hit [ADC]
    float sumadc   = float(-1.f);    //!< "SummedADC" -- sum of ADC values under gaussian fit [ADC]
    float width    = float(-1.f);    //!< Width of fitted gaussian hit [ticks]
    float goodness = float(-1.f);    //!< Goodness-of-fit of fitted gaussian hit
    float time     = float(-1.f);    //!< Peak time of hit [ticks]
    Vector3D sp;                     //!< Space-Point Position of hit [cm]
    int id = int(-1);                //!< ID of hit
    ...
    int16_t start = int16_t(-1);     //!< Start tick of hit [ticks]
    int16_t end   = int16_t(-1);     //!< End tick of hit [ticks]
  ...
  struct TrackHitInfo {
    ...
    float pitch = float(-1.f);         //!< Pitch of track across wire the hit is on [cm]
    float dqdx  = float(-1.f);         //!< Initial computed dq/dx of hit [ADC/cm]
    float rr    = float(-1.f);         //!< Residual range of hit along track [cm]
  ...
  struct MetaInfo {
    int run    = int(-1); //!< Run number 
    int evt    = int(-1); //!< Event number 
    int subrun = int(-1); //!< Subrun number
    uint64_t time;        //!< Timestamp
    int ifile  = int(-1); //!< Index of file into processing
    int iproc  = int(-1); //!< Index of process number into processing (useful for grid)
  };

  struct TrueHit {
    int16_t cryo  = int16_t(-1);  //!< Cryostat of hit
    int16_t tpc   = int16_t(-1); //!< TPC of hit
    int16_t plane = int16_t(-1); //!< Plane of hit
    int wire    = int(-1);       //!< Wire of hit
    int channel = int(-1);       //!< Channel of hit

    unsigned ndep = unsigned(0); //!< Number of depositions in hit
    float nelec = float(0.f);    //!< Number of electrons in hit
    float e     = float(0.f);    //!< energy in hit [MeV]
    float pitch = float(0.f);    //!< Track pitch for hit, using true direction [cm]
    float pitch_sce;             //!< Track pitch for hit, after distortion to pitch caused by space charge [cm]
    
    float rr = float(0.f);    //!< Track residual range for hit [cm]
    int itraj = int(-1);      //!< Index of hit along trajectory
    Vector3D p               = {0., 0., 0.}; //!< Location of hit, computed after space charge [cm]
    Vector3D p_scecorr       = {0., 0., 0.}; //!< Location of the hit after un-doing space charge [cm]
    Vector3D p_width         = {0., 0., 0.}; //!< Width of depositions going into hit [cm^2]
    Vector3D p_scecorr_width = {0., 0., 0.}; //!< Width of depositions going into hit after un-doing space charge [cm^2]
    float time = float(0.f);  //!< Time of hit [ticks]
  ...
  struct TrueParticle {
    ...
    unsigned plane0nhit = unsigned(0);  //!< Number of hits on plane 0 (1st Ind.)
    unsigned plane1nhit = unsigned(0);  //!< Number of hits on plane 1 (2nd Ind.)
    unsigned plane2nhit = unsigned(0);  //!< Number of hits on plane 2 (Col.)
    ...
    int   wallin  = int(-1);     //!< Wall of cryostat particle enters (wNone if starting in detector)
    int   wallout = int(-1);     //!< Wall of cryostat particle exits (wNone if stopping in detector)
    ...
    int       pdg = int(-1);         //!< Particle ID code
    int      G4ID = int(-1);         //!< ID of the particle (taken from G4 -- -1 if this particle is not propogated by G4)
    int    parent = int(-1);         //!< ID of parent particle
    
    int   start_process = int(-1); //!< Start G4 process of the particle. Values defned as enum in StandardRecord
    int   end_process   = int(-1); //!< End G4 process of the particle. Values defined as enum in StandardRecord
  ...
  struct TrackInfo {
    ...
    float xShiftCRT = float(0.f);   //<! If whicht0 == 1,2,3 the amount by which the track was shifted in x
    int id       = int(-1);         //!< ID of track
    int cryostat = int(-1);         //!< Cryostat number of track
    ...
    float length = float(-1.f);     //!< Length of track [cm]
    ...
    float const_fit_C = float(-1.f); //!< Fit parameter
    float const_fit_residuals = float(-1.f); //!< Fit parameter

    float exp_fit_A = float(-1.f); //!< Fit parameter
    float exp_fit_R = float(-1.f); //!< Fit parameter
    float exp_fit_residuals = float(-1.f); //!< Fit parameter

    int n_fit_point = int(-1); //!< Fit parameter

    int selected  = int(-1); //!< Index of the tool that selected this track
    int nprescale = int(-1); //!< Prescale of the tool that selected this track
