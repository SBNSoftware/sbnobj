...
/** True energy depositions 
  ...
  */
  struct TrueBlip {
    ...
    geo::Point_t  Position;               ///<  Charge weighted true-XYZ position [cm] 
 ...
 /** Hit clusters are collections of adjacent hits on a single wire plane.
  ...
  struct HitClust {
    ...
    bool    isMerged        = false; ///< Deprecated internal flag for tracking blips before/after merge. 
    ...
    /**
     * @brief Total charge of hits making up the hit cluster [e-]
     * 
     * Charge is reconstructed from calo::CalorimetryAlg::ElectronsFromADCArea().
     * In SBND, configuration is in `sbnd_calorimetryalgmc`
     * from `sbndcode/LArSoftConfigurations/calorimetry_sbnd.fcl`.
    */
